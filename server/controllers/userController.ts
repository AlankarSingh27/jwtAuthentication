...
import {APP_STATUS} from "../contants";
        ...
        if (secretKey && payload) {
            jwt.sign(payload, secretKey, {
                ...
                if (encoded) {
                    ...
                    response.cookie("hello", encoded, {
                        expires: new Date(Date.now() + 3600000), // set the cookie expiration time
                        httpOnly: true// make the cookie accessible only by HTTP requests, not by client-side scripts
                    });
